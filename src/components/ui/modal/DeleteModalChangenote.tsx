...
import { Changenote } from "@/components/types/Changenote";

export default function DeleteModalChangenote({
  changenoteData,
  onDelete,
}: {
  ...
  const handleDelete = async () => {
    ...
    const response = await changenoteService.delete(
      changenoteData.id,
      authToken,
    );
    if (response.status !== 200) {
      ...
      setIsSuccess(true);
      onDelete(String(changenoteData.id));
      setIsToastOpen(true);
    }
    setIsLoading(false);
  };
  ...
  return (
    <div>
      ...
      <dialog id="delete_modal_changenote" className="modal">
        <div className="modal-box w-fit p-10 bg-white">
          ...
          <p className="mb-4 text-center text-black/70">
            Are you sure to delete this changenote?
          </p>

...
import { logService } from "@/data/service";
import { formatCustomDate } from "@/lib/formatDate";
import DeleteModalLog from "../ui/modal/DeleteModalLog";
...
export default function LogTable({
  ...
  getEditedData: (editedData: LogMaintenance) => void;
  handleFormShowing: (state: boolean) => void;
}) {
  const [logs, setLogs] = useState<LogMaintenance[]>([]);
  const [deletedData, setDeletedData] = useState<LogMaintenance>();
  ...
  const loadData = async () => {
    ...
    const response = await logService.getAll(idDevice, authToken);
    if (response.data) {
      const logsData: LogMaintenance[] = response.data;
      setLogs(logsData);
    } else {
      setIsToastOpen(true);
      setErrorData({
  ...
  const handleEditBtn = (editedData: LogMaintenance, isShow: boolean) => {
    getEditedData(editedData);
    handleFormShowing(isShow);
  };

  const handleDeleteBtn = (logData: LogMaintenance) => {
    setDeletedData(logData);
    (
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {logs.map((log, i) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                        <p>
                          {formatCustomDate(log.createdAt, "yyyy/MM/dd")}
                        </p>
                      </TableCell>
                      <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                        <p>{log.teknisi}</p>
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                        {/* <button
                          ...
                          data-tip="Edit"
                          onClick={() => handleEditBtn(log, true)}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"

...
import { addLogAction, editLogAction } from "@/data/actions/FormLogActions";

const INITIAL_STATE = {
  data: null,
};

export default function FormLogMaintenance({
  initLogData,
  getIsFormInitShowingState,
  onClose,
}: {
  initLogData?: LogMaintenance;
  ...
  const [formInitState, setFormInitState] = useState<LogMaintenance>({
    id: initLogData?.id || 0,
    id_device: initLogData?.id_device || "",
    createdAt: initLogData?.createdAt || "",
    teknisi: initLogData?.teknisi || "",
    keterangan: initLogData?.keterangan || "",
    opsi: initLogData?.opsi || "",
  });
  ...
  return (
    <ComponentCard title={`${initLogData ? "Edit" : "New"} Record - Log Maintenance`}>
      <ModalLoading isOpen={isLoading} />
      <SuccessModal message={formAddState?.message || formEditState?.message} />
      <form
        action={initLogData ? formEditAction : formAddAction}
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="col-span-full">
            ...
            <fieldset className="fieldset w-full">
              ...
              <TextArea
                name="keterangan"
                placeholder="Isi keterangan tambahan disini..."
                value={formInitState.keterangan}
                onChange={handleInputChange}
                required={true}
              />
              <span className="fieldset-label text-gray-800 dark:text-white/90">
                <div className="flex flex-col gap-3">
                  <ZodErrors
                    error={
                      formAddState?.zodErrors?.keterangan ||
                      formEditState?.zodErrors?.keterangan
                    }
                  />
                </div>
              </span>
            ...
            <fieldset className="fieldset w-full">
              <Label htmlFor="opsi">
                Opsi Pemeliharaan <span className="text-red-500">*</span>
              </Label>
              <TextArea
                name="opsi"
                placeholder="Isi opsi pemeliharaan disini..."
                value={formInitState.opsi}
                onChange={handleInputChange}
                required={true}
              />
              <span className="fieldset-label text-gray-800 dark:text-white/90">
                <div className="flex flex-col gap-3">
                  <ZodErrors
                    error={
                      formAddState?.zodErrors?.opsi ||
                      formEditState?.zodErrors?.opsi
                    }
                  />
                </div>
              </span>
